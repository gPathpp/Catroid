...
public class GoToRandomPositionAction extends TemporalAction {
	...
	protected void update(float percent) {
		randomXPosition = (float) Math.random()
				* (ScreenValues.Companion.getSCREEN_WIDTH() + 1) - (ScreenValues.Companion.getSCREEN_WIDTH() / 2);
		randomYPosition = (float) Math.random()
				* (ScreenValues.Companion.getSCREEN_HEIGHT() + 1) - (ScreenValues.Companion.getSCREEN_HEIGHT() / 2);
